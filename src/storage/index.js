...
// 引入 getPluginsDir
const { getBaseDir, getPluginsDir, getConfigDir, getPluginConfigDir, getPluginDataDir, getAuthCacheDir } = require('../utils/paths');

class Storage {
    constructor(dataDir) {
        this.baseDir = getBaseDir();
        this.pluginsDir = getPluginsDir(); // 这里的 pluginsDir 指向根目录下的 plugins
    ...
    ensureDirectories() {
        // 自动创建根目录下的 plugins 文件夹
        if (!fs.existsSync(this.pluginsDir)) {
            try {
                fs.mkdirSync(this.pluginsDir, { recursive: true });
                // 可选：创建一个说明文件，告诉用户这里放什么
                fs.writeFileSync(
                    path.join(this.pluginsDir, 'README.txt'),
                    'Put your .js plugin files here.\n将你的 .js 插件文件放在这里。'
                );
            } catch (err) {
                console.error('Failed to create plugins directory:', err.message);
            }
        }
